...
- **Testing**: All new code requires tests — project standard is 2430 logical invocations passing (1435 Java `@Test` methods across 244 files + 789 Jest `it/test` blocks across 91 files + 78 top-level Go `Test*` funcs across 10 files + 80 Playwright `test()` blocks across 18 specs + 48 MCP-server vitest `it/test` blocks across 8 files under `mcp-server/`). Multiple Java files use Testcontainers (real Postgres + RLS). Counts are the single source of truth in `docs/metrics.json`, enforced by **three** gates, one per link in the chain. Two live in `.github/workflows/docs-freshness.yml`: `scripts/docs-freshness.sh` (source tree → `docs/metrics.json`) and `scripts/check-doc-metrics.sh` (the numbers quoted in prose here, in `CLAUDE.md` and in `README.md` → `docs/metrics.json`). Both fail the build on drift. The second exists because the first never opened a doc: README sat at `921` for months while the tree was at `1895`, and `docs-freshness.sh` was green on every one of those commits. The third is `scripts/check-test-count-oracle.sh` in `ci-cd.yaml` (issue #291), which asks Jest, Playwright and vitest **themselves** — because the first two only ever agreed with each other, and stayed green over a Jest figure no runner ever produced: the old counter grepped for a token that also matches `RegExp.prototype.test(`, and was blind to table-driven `.each` blocks entirely. The JS/TS families are now counted by `scripts/count-test-blocks.mjs`, which exits 2 rather than emit a number it cannot derive; its own refusals are falsified by `scripts/check-test-block-counter.sh`.
...
- AWS SDK v2 (2.50.2) - S3 API for image storage
