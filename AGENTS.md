...
- **Testing**: All new code requires tests — project standard is 1675 logical invocations passing (1135 Java `@Test` methods across 201 files + 376 Jest `it/test` blocks across 58 files + 77 top-level Go `Test*` funcs across 9 files + 40 Playwright `test()` blocks across 11 specs + 47 MCP-server vitest `it/test` blocks across 8 files under `mcp-server/`). Multiple Java files use Testcontainers (real Postgres + RLS). Counts are the single source of truth in `docs/metrics.json` and are enforced by the `docs-freshness` CI gate (`.github/workflows/docs-freshness.yml`, script `scripts/docs-freshness.sh`), which fails the build on drift.
