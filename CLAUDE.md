...
- **Testing**: All new code requires tests — project standard is 1166 logical invocations passing (834 Java `@Test` methods across 133 files + 229 Jest `it/test` blocks across 32 files + 75 top-level Go `Test*` funcs across 8 files + 28 Playwright `test()` blocks across 6 specs). Multiple Java files use Testcontainers (real Postgres + RLS). Counts are the single source of truth in `docs/metrics.json` and are enforced by the `docs-freshness` CI gate (`.github/workflows/docs-freshness.yml`, script `scripts/docs-freshness.sh`), which fails the build on drift.
