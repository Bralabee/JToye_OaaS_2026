...
- **Testing**: All new code requires tests — project standard is 1192 logical invocations passing (856 Java `@Test` methods across 138 files + 231 Jest `it/test` blocks across 32 files + 77 top-level Go `Test*` funcs across 9 files + 28 Playwright `test()` blocks across 6 specs). Multiple Java files use Testcontainers (real Postgres + RLS). Counts are the single source of truth in `docs/metrics.json` and are enforced by the `docs-freshness` CI gate (`.github/workflows/docs-freshness.yml`, script `scripts/docs-freshness.sh`), which fails the build on drift.
