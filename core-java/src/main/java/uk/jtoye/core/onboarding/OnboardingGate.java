 ...
 *
 * <p>{@link #type()} maps the gate to its {@link GateType} row, {@link #mandatory()}
 * decides whether it blocks approval, {@link #isAutomatic()} marks gates the async
 * runner evaluates itself (vs. webhook/manual gates that only get materialised),
 * and {@link #evaluate(VendorOnboarding)} performs the automatic check.
 */
public interface OnboardingGate {
    ...
    /** {@code true} if this gate must be PASSED/WAIVED before approval. */
    boolean mandatory();
