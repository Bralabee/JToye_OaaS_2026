...
class AuditIntegrationTest {
    ...
    void shouldTrackProductUpdatesInAuditHistory() {
        ...
        // Re-assign: saveAndFlush merges the detached entity and bumps @Version on
        // the RETURNED instance only — reusing the stale reference would carry
        // version=0 into the next update and throw OptimisticLockException.
        product = productRepository.saveAndFlush(product);
    ...
    void shouldIsolateAuditHistoryByTenant() {
        ...
        shopA1.setSlug("it-" + java.util.UUID.randomUUID().toString().substring(0, 8));
        ...
        shopA2.setSlug("it-" + java.util.UUID.randomUUID().toString().substring(0, 8));
        ...
        shopB.setSlug("it-" + java.util.UUID.randomUUID().toString().substring(0, 8));
