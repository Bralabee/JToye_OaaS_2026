...
- **Nine of eighteen checks silently stopped running, and the nightly had been red for
  five consecutive nights (2026-09-07 through 09-11).**
  `frontend/e2e/cart-identity-boundary.verify.mjs` located the storefront Add button with
  an anchored `/^add$/i`. #726 (A11Y-4, WCAG 2.4.6) gave each card a
...
allergen picture in front of both the customer and the kitchen, and closes LGL-02 with a CI
accessibility gate that has been shown to fail against a deliberately broken control.

#### Consumer safety
- **The allergen set an order carries is snapshotted at write time, not joined at read time**
  (V63 `order_items.allergen_mask` + `allergen_flag_mask`). Under a live join a vendor editing a
  product after the order silently rewrites what the customer acknowledged, and the "not recorded"
  state disappears entirely (31-10, LGL-03).
- **Checkout shows the set before the customer commits and refuses to submit without an explicit
  acknowledgement** — the refusal announces itself via `role="alert"` and moves focus to the control
  that refused. All three of `aria-invalid`, `aria-describedby` on a field, and a programmatic
  focus-move had zero precedent in this codebase (31-14).
- **The kitchen display carries an order-level banner, per-item badges and a monochrome print block**,
  so the warning survives to the moment it is acted on (31-15).
- **Three states are distinguished, not two** — declared allergens, declared none, and NOT RECORDED.
  Rendering absent data as nothing would itself be an allergen-free claim. No backfill was performed,
  deliberately: 572 historic order lines correctly read NULL (31-10, 31-14, 31-15).
- The vendor's **declared mask and the advisory reconciliation flag stay structurally separate**; a
  text heuristic never rewrites a vendor's allergen statement (31-04, 31-10).

#### Legal surface (LGL-01)
- **Five policy routes live and reachable** — `/legal` plus privacy, cookies, retention and
  accessibility — linked from one `PublicFooter` column that reaches every public route including
  tenant storefronts, because `app/shop/layout.tsx` already renders that footer over `/shop/**`
  (31-08, 31-11, 31-12, 31-13, 31-17).
- **The cookie policy is exhaustive over measured storage, not over the plan's figures.** The tree
  held 5 sessionStorage keys rather than 2 (three are constants, invisible to a literal search) and
  9 localStorage keys rather than 8; a drafted claim of "zero third-party scripts" was **false** —
  Stripe loads at checkout with `js.stripe.com` in the CSP (31-11).
- **Retention schedule published from `docs/retention-manifest.json`**, with `claims.manifest` rules
  that FAIL when a published sentence is deleted, and a table that fits 375px without duplicating the
  DOM (31-06, 31-12).
- **UK-GDPR DSAR intake and cross-tenant fan-out** (V62), plus the Article 26 arrangement, behind a
  published single point of contact (31-05, 31-07, 31-09).
- **Client-only cookie-consent gate** — no table, no migration — proven in both directions with a
  fixture category, because a gate over zero categories cannot fail as shipped (31-16).

#### Accessibility (LGL-02)
- **A dated partial-conformance statement** naming seven exceptions, each with a remediation date,
  driven by a declared constant so a past review date reds the build (31-13).
- **A per-PR axe gate over 13 declared surfaces on both viewports**, in the only browser job that
  blocks a merge. Its break arm produced a real `image-alt [critical]` naming route, rule and node
  (31-18).
- **Zero-violation results are treated as artefacts until proven otherwise.** Two were reproduced and
  quantified: an unseeded checkout scans 8 elements with no `<h1>` (0 violations / 22 passes) and a
  storefront with the dish modal unopened scans 0 dialogs (0 violations / 23 passes). The gate seeds
  the basket and opens the modal (31-18).
- **Two contrast defects found and fixed**: the policy TOC at 4.41:1 across all four policy documents,
  mobile only; and `/shop`'s delivery-info line at 2.56:1, caught by the new gate on its first CI run
  (31-18).

#### Notes
- Test manifest reconciled at phase close: total logical invocations 2951 → 3185 (Jest 1008 → 1230
  across 106 → 120 files; Playwright 101 → 113 across 20 → 22 specs), regenerated by
  `scripts/docs-freshness.sh --write` with the prose in `CLAUDE.md`/`AGENTS.md`/`README.md` updated.
- One new gate script this phase (36 → 37): `check-retention-enforcement.sh`. The manifest-to-prose
  half needed no new script and rides the existing `claims.manifest` engine.
- **`DSAR_VERIFY_BASE_URL` was defaulting to `localhost` with no manifest supplying it** — outside a
  laptop the link emailed to a data subject was dead, and silently so. Now wired per environment from
  `app-config`. Found by CI on this branch's first contact with it.
- **Known limitation, recorded not hidden:** the allergen acknowledgement gates the client submit
  only; a direct API call bypasses it. Server-side enforcement is follow-up work.
- **Pre-publication requirement:** `privacy@olajay.co.uk` is published as the data-subject contact
  route, but its existence and monitoring cannot be verified from this repository.

<!-- MERGE STEP for the Phase 28 entry below: append this phase's PR number as "(#NNN)" to its
     heading when the PR is opened. No number is invented here (the PR does not exist yet) —
     check-changelog-cites-pr SKIPs off a pull request and only asserts the citation at PR time, and
...
(A1 falsified, B1 fixed with #552 closed, the rest closed or accepted with dated evidence).

#### Security
- **A1's root cause falsified, its real guard proven** — the two shop-content tables the finding
  named carry a tenant column and ENABLE/FORCE RLS; the cross-tenant write is blocked at the
  service-layer ownership gate, shown able to fail (28-01).
- **The served OpenAPI document drops the dev-only tenant-override header on deployed profiles** —
  asserted on the document springdoc actually serves, with a filter-present control (28-02, SEC-03).
- **`auth == null` no longer grants internal trust** — it is now an explicit `asSystem` declaration
  with a behavioural guard that reds if a background path reaches a gated call undeclared (28-06).
- **A revoked KDS operator's open SSE stream now delivers nothing** — a per-emit shop-grant re-check;
  the idle socket lingers at most 5 minutes carrying no events, and the STOMP equivalent is filed
  (28-04, #281).
- **The application runs as a non-owner database role** — a new DML-only `jtoye_runtime` with
  FOR-ROLE future-object grants and a boot-time validator that refuses to start as a table owner, so
  tenant isolation no longer depends on FORCE being remembered on every future table (28-07, 28-08).
- **All six local credentials read during the test were rotated on the running stack** — each proven
  superseded-refused / current-accepted, with one Keycloak import also removing an unused public realm
  client (11 → 10 clients) (28-05, 28-10). #552 closed.

#### Hardening
- **The MinIO bootstrap is digest-pinned** and its anonymous bucket policy narrowed to GetObject-only,
  closing anonymous object enumeration while keeping storefront images anonymously readable by URL
  (28-09). #626, #270 closed.
- **A media Content-Type gate** (`check-media-content-types.sh`) holds every stored object inside the
  upload allowlist — measured 0 of 768 objects outside it and 0 of 37 legacy objects carrying EXIF
  (28-03).

#### Notes
- Test manifest reconciled at phase close: `docs/metrics.json` total logical invocations 2769 → 2807
  (the whole delta is Java: +38 `@Test` methods across +8 files), regenerated by
  `scripts/docs-freshness.sh --write` with the prose in `CLAUDE.md`/`AGENTS.md`/`README.md` updated.
- Two new CI gates added this phase (34 → 36 gate scripts): `check-media-content-types.sh` and
  `check-pentest-triage.sh`.
- The remediation lives on the `phase/28-security-triage` branch; the dev stack is already remediated
  live, and `#552` is closed at phase close.
...
- Test counts move: +3 `@Test` in `UnsubscribeLinkRoutingTest`, +3 in
  `UnsubscribeLinkReachesControllerIntegrationTest`, +3 in `NotificationPropertiesBindingTest`, +4 in
...
### `PUT /shops` told the client "saved" and threw the instruction away (#450 item 4) (#573) — 2026-08-05
...
- **The `key` on each board branch is load-bearing.** React reconciles a conditional by position, so without distinct keys the four states are one node — and a node that *persists* while moving is an unstable element, whereas a freshly-mounted one is not. Measured in the fail direction, on a real build served the same way: reserve reverted to 16rem → **0.1981**; keys removed with the reserve kept → **0.1343**. Both breach the budget, both CLEAN brackets read 0.0171, and neither arm returns all the way to 0.8287 because the other fixes are still in place.
...
**The mechanism already existed and had never been switched on.** #442 built the management listener: edge-go serves `/metrics` on a separate port when `EDGE_MANAGEMENT_PORT` is set, and on the application port when it is not. The variable was set **nowhere** — not in compose, not in k8s. So the gap was configuration, and the fix is a value plus the scrape target that has to move with it.

#### Fixed
- **`/metrics` moves to a management listener on compose.** `docker-compose.full-stack.yml` sets `EDGE_MANAGEMENT_PORT` on the edge-go service; the port is deliberately **not published** to the host, because Prometheus reaches it over `jtoye-network` by container name and publishing it would recreate the exposure one port to the left. `/health` and `/ready` stay on the application port — the kubelet probes target them there and moving them would fail every rollout.
- **The scrape target moves in the same change, from the same key.** `prometheus.yml.tmpl` gains `__EDGE_GO_METRICS_PORT__`, rendered by `prometheus/entrypoint.sh` — the idiom `CORE_JAVA_METRICS_PORT` already uses. Both compose files interpolate the **one** `.env` key `EDGE_GO_METRICS_PORT` (default `9101`), so the listener and the target cannot drift apart. They must not: the edge serves `/metrics` on exactly one of the two ports, so a mismatch is a permanently-DOWN target, not a duplicate scrape.

#### Added
- **A test for the routing decision, which is what was actually wrong.** `metrics_test.go` builds its own router and calls `r.GET("/metrics", metricsHandler())` directly — it tests the handler, so it was green throughout the exposure and would have stayed green after a fix that did nothing. The new `metrics_routing_test.go` asserts the decision on the engine's own route registry: management port set → `GET /metrics` absent from the application router, unset → present, `/health` present in both. Shown to fail against the inverted wiring before being trusted.
- **`EDGE_MANAGEMENT_PORT` is validated at boot.** `resolveManagementPort` trims, rejects anything that is not an integer 1-65535, and `main` turns the error into a Fatal naming the variable. A malformed value already removes `/metrics` from the application router, so limping on would have left the gateway with no scrape endpoint and no explanation.

#### Notes
- **k8s deliberately keeps the old topology, and the recorded reason for that has been rewritten because the old one is now false.** `k8s/scripts/check-env-contract.sh` justified the omission with "that is exactly where the Phase 27 scrape config targets it with no credentials" — true until this change, false for compose after it. The conclusion holds for a different reason: **nothing scrapes edge-go in k8s at all** (no monitoring manifests, DPLY-03), so the only consumer of a port there would be the `prometheus.io/*` annotations on `k8s/base/edge-go-deployment.yaml`, which name 8080 and which no controller in this repo reads. Setting the variable in k8s alone would make those annotations lie for no scraper. It belongs in one change with DPLY-03.
- **The entrypoint's port validation is now one function used by both ports**, and it uses a `case` glob rather than a piped `grep`: this runs under busybox ash where a here-string is unavailable, and a piped `grep -q` is the shape that inverts under `pipefail`. The length guard is load-bearing — `[ "$v" -gt 65535 ]` *errors* on a value wider than an int64 instead of rejecting it. Both directions run in the real `prom/prometheus:v2.48.0` image: valid ports render, pass `promtool check config` and exec Prometheus (rc 0), while `abc`, `99999` and a 20-digit value each exit 1 with the variable named, and a template placeholder the `sed` line does not cover exits 1 rather than scraping a host literally called `__EDGE_GO_SCRAPE_PORT__`.
- **A template edit needs the Prometheus container RECREATED, not restarted.** The render happens at container start; `up -d --force-recreate prometheus` is the only form that picks up a changed environment value. A verifier who restarts reads a stale render and gets a false pass.
### An unconfigured WhatsApp webhook reported itself as an internal error (#450 item 3) (#575) — 2026-08-05

With `WHATSAPP_APP_SECRET` unset the edge refuses the webhook — correct, and unchanged — but it answered **500**, which says *the edge broke*. It had not: this is a known, named, operator-fixable state. It is now **503 with `Retry-After: 300`**.

Explicitly **not 501**. That would advertise the endpoint as unimplemented, which is false — it is implemented and deployed — and worse, because a caller may reasonably stop calling a 501 for good.

#### Fixed
- **`edge-go/cmd/edge/handlers.go`** — the unset-secret branch returns 503 and sets `Retry-After`. The refusal happens before the body is read and before anything downstream is touched, exactly as before; **only the status changed**.
- **The OpenAPI contract moved with the code.** `@Failure 500` becomes `@Failure 503`, plus an `@Header 503 {integer} Retry-After`, and the three generated `edge-go/docs/*` artifacts are regenerated. The description now states the two refusals separately — 401 for an absent/invalid signature, 503 for an unset secret — because "fail-closed" alone did not tell a caller which it would get.
- **`k8s/scripts/check-env-contract.sh`** — the `WHATSAPP_APP_SECRET` allowlist entry asserted the handler "refuses the webhook with 500" and cited `handlers.go:209-213`. Both halves were about to be false. Corrected to 503 and to `:249-255`, verified by reading the range back. Worth noting where this rot lives: `check-doc-citations.sh` scans seven docs and a **shell script is not one of them**, so nothing would have caught it.

#### Notes
- **`Retry-After` is a constant, not an env var.** The value does not vary by environment — it encodes what kind of wait this is (an operator setting a secret, not a load spike). 300s is long enough that a client honouring it stops hammering an endpoint that cannot succeed, short enough that recovery is prompt once the secret lands. An env name would have to be justified to the k8s env-contract gate by every runtime, to tune a number nobody has a reason to tune. Meta runs its own retry schedule and is not expected to honour the header; it is there for correctness and for well-behaved clients.
- **Fails closed is now asserted, not assumed.** The flipped test wires a Core stub that counts requests and asserts **zero**, so an edit that moved the secret check below any processing fails the test rather than quietly turning a status-code fix into an open door. The four signature arms (401 missing, 401 invalid, 200 valid-but-unconfigured, and the happy path) are the untouched control and stayed green throughout.
- **The status disclosure is unchanged.** An unauthenticated caller can tell "configured" from "not configured" — but could already, via the 500. Returning 401 to hide it would lie to the operator whose only diagnostic is this response.
- **Both directions, on the real instruments.** Before: `TestOpenAPISpec_Fresh` PASSED, proving the freshness check was live locally rather than skipping for a missing `swag`; after the annotation edit and before regeneration it FAILED with "docs/swagger.json is stale"; after `swag init`, green. The behaviour test was run against the pre-fix handler and failed on the status; the fails-closed assertion was run against a handler that reached Core and failed on the call count.
- **One test function changed, none added** — deliberately, so `docs/metrics.json` needs no re-baseline for this change.
...
### Card checkout has never worked, only been unreachable — the PaymentIntent was created before the order existed (#539 — closes #538) — 2026-08-05

Guest checkout returned **HTTP 500 on every environment configured with a Stripe API key**. `PublicStorefrontService.createGuestOrder` created the PaymentIntent *before* `orderRepository.save`, so `PaymentService.createPaymentIntent` dereferenced a null `Order.getId()` while stamping the intent's `order_id` metadata. This is pre-existing and predates the 2026-08-04 merge train; no PR in it touched payment code.

It stayed invisible for a specific reason worth recording: `isConfigured()` is `apiKey != null && !apiKey.isBlank()`, **no deployed stack sets `STRIPE_API_KEY`**, and so every environment silently took the Cash-on-Delivery fallback. The first successful nightly E2E run (`gh run view 30955236660`) was the first thing ever to execute the card branch.

#### Fixed
- **`core-java/.../storefront/PublicStorefrontService.java`** — persist, then pay. `saveAndFlush` runs before the Stripe call, so the row has an identity and the V24 `(tenant_id, idempotency_key)` partial unique index is resolved against Postgres *before* money is asked for — a racing duplicate checkout is now a DB rejection rather than a second PaymentIntent.
- **`core-java/.../payment/PaymentService.java`** — `createPaymentIntent` refuses a transient order outright (`IllegalStateException`), so a reintroduction of this ordering is a named failure that never reaches Stripe rather than a `NullPointerException` inside a metadata builder.

#### Added
- **`core-java/.../payment/PaymentIntentResult.java`** — the PaymentIntent id is now returned alongside the client secret and **persisted as `order.payment_reference`**. It never was before, which left the WR-02 idempotent-retry re-fetch (`PublicStorefrontService:346-357`) unreachable — it is guarded on `paymentReference != null` — so a retried card checkout came back with a null client secret and an order the customer could never pay for. It also gives an unpaid order a local column to reconcile against Stripe.
- **`Idempotency-Key` on `PaymentIntent.create`**, derived from the order id. `Stripe.maxNetworkRetries` defaults to `2` (stripe-java 33.2.0, `com/stripe/Stripe.java:29`) and the SDK emits the header only when the caller supplies one — it never generates one (`com/stripe/net/StripeRequest.java:335`) — so one checkout could leave up to three PaymentIntents at Stripe. Only one client secret ever reaches a browser, so this was duplicate-intent noise rather than a guaranteed double charge.
- **`core-java/.../storefront/GuestCheckoutOnlinePaymentIntegrationTest.java`** — the first test in the repository to execute the online-payment branch with the **real** `PaymentService` bean against real Postgres.

#### Notes
- **Why 1,425 Java tests missed it.** Every existing test of the `isConfigured() == true` branch mocks `PaymentService` — `PublicStorefrontServiceTest` stubs `createPaymentIntent(...)` to return `"cs_test_secret"` — and the NPE lives *inside* the real service, so the mock sat exactly on top of the defect. The new test mocks only the outermost Stripe HTTP call (`MockedStatic` on `PaymentIntent.create`) and flips `isConfigured()` by mutating the injected `StripeProperties` bean, so everything between the storefront and the network genuinely runs.
- **Falsifiable in both directions, same test source.** `git hash-object` of the new test is `828ab191` before *and* after the fix. Unfixed: 2 of 3 fail, reproducing the CI stack verbatim (`NullPointerException ... at PaymentService.java:126 ... at PublicStorefrontService.java:512`). Fixed: 3 of 3 pass. **The COD control passes in both directions**, which is what makes it a control.
- **A failed intent rolls the order back, and the transaction semantics were verified rather than assumed.** `createGuestOrder` is method-level `@Transactional` over a class-level `readOnly = true`, and `PublicStorefrontController` carries **no** `@Transactional` — so this is the outermost boundary and the existing unchecked rethrow already discards the DRAFT row. That is also the behaviour we want: keeping the row would be strictly worse, because it carries the customer's idempotency key, so their retry would hit the short-circuit and get back an order with no client secret — unpayable forever. If Stripe created the intent and the failure was on the response leg, that intent is orphaned but harmless: its client secret never reaches a browser, so it is never confirmed and expires uncaptured.
- **Five break arms, each restored and verified by content hash, with a closing clean arm.** Disabling rollback → `orders surviving a failed PaymentIntent = 1`. COD writing `PaymentStatus.PENDING` → control red. Dropping `setPaymentReference` → `expected "pi_issue538_stub" but was null`. Removing the `saveAndFlush` → **4 unit tests** red *and* 2 integration tests red on the new guard, proving the regression is now caught at unit speed. A constant idempotency key → the derived-from-order-id assertion red.
- **Still unproven against Stripe, and that is stated rather than implied.** This makes the card path correct by construction and by test; it does not make it observed working. No live key exists anywhere in the change — the integration test's api-key value is a plain sentence deliberately not shaped like a credential, and no transport is ever constructed. The artifact that would close the gap is a test-mode transaction receipt (a real `pi_...` with the matching local order row), which needs a test-mode key and is a decision for the repository owner.
- **Deliberately not done:** the failure still surfaces as a bare `RuntimeException` → HTTP 500. A typed RFC 7807 503 would serve the agent-readiness contract better, but it changes the status code the checkout UI reacts to, and this change is scoped to `core-java`.
...
The two issues are **one change, and the order was forced**: a `"use client"` module cannot export `generateMetadata`, so the discoverability work was not merely missing, it was unimplementable until the routes rendered on the server.

#### Changed
- **`frontend/app/shop/page.tsx`, `frontend/app/shop/[slug]/page.tsx`** — now server components that fetch and render. The interactive half moved unchanged into `shop-discovery-client.tsx` / `shop-detail-client.tsx`, which receive the server's result as a seed and skip their mount fetch. `"use client"` was never the problem on its own — a client component *is* rendered to HTML by the server; fetching in a `useEffect` is what guaranteed that HTML was empty, because an effect only ever runs in a browser.
- **`frontend/app/sitemap.ts`** — shop pages are now included, and the base URL is resolved at runtime. See the sitemap note below; the reversal is deliberate and argued, not silent.
- **`frontend/app/shop/layout.tsx`** — its single static title was the reason `/shop` and all three storefronts agreed on one `<title>`; it now supplies `metadataBase` and remains only as the fallback for the storefront's utility routes.

#### Added
- **`frontend/app/robots.ts`** — `/robots.txt` returned **404**. It now returns 200, points at the sitemap with an absolute URL, and disallows the authenticated and mid-journey routes while leaving `/shop` and `/shop/<slug>` crawlable.
- **`frontend/lib/storefront-server.ts`** — server-side loaders with a **three-valued** result (`ok` / `notfound` / `defer`). `defer` exists so a 429 is never presented as an authoritative answer: the server hands over to the island, which already owns the retry budget and the empty-state policy (F-RATE / #88).
- **`frontend/lib/structured-data.ts`** — schema.org builders: `Restaurant` (a LocalBusiness subtype) with `hasMenu`, an `ItemList` of `Product`+`Offer` per dish, and a `BreadcrumbList`.
- **`frontend/lib/opening-hours.ts`** — one `isOpenNow`, replacing two. See the correctness note below.
- **`frontend/app/shop/[slug]/not-found.tsx`** — the same "Shop not found" screen, moved so it can carry `robots: noindex`.

#### Notes
- **Measured in the SERVED BYTES, before and after, because that is the whole point.** A browser assertion would pass identically on the pre-fix tree — the client fetch fills the DOM in ~2.5s and Playwright waits. `/shop/brixton-village-grill`: **34,435 → 91,745 bytes**, `<h1>` **0 → 1**, occurrences of the shop's own name **0 → 33**, `rel=canonical` **0 → 1**, `og:` **0 → 8**, `twitter:` **0 → 4**, `application/ld+json` **0 → 2**. `/shop`: shop-name occurrences **0 → 15**. `/robots.txt`: **404 → 200**. `<title>` was one shared string across 4/4 public storefront cells and is now unique per route. The landing page `/` was carried through both runs unchanged (62,503 bytes, 0 canonical/og/twitter/JSON-LD) as a negative control proving the probe is not matching everything.
- **Falsifiable in both directions.** The new spec is **14/14 green** against this branch and **14/14 red** against the unmodified running stack. The JSON-LD validator exits **0** here and **1** there.
- **The LCP artefact #507 warned about is real, and both numbers are reported.** At the repo's throttled mobile profile (390px, 4× CPU, ~Fast 3G, 3 runs, medians): `/shop` **LCP 712 → 732 ms** (flat — the pre-fix LCP element was the *skeleton*) while **time-to-content 2456 → 916 ms** and browser-side catalogue calls **1 → 0**. `/shop/brixton-village-grill` **LCP 2316 → 736 ms**, **time-to-content 2431 → 913 ms**, calls **6 → 0**.
- **CLS regressed and is reported rather than omitted:** `/shop` **0.0017 → 0.0115**, `/shop/[slug]` **0.0044 → 0.0285**. Both remain ~3.5× inside the 0.1 budget. The shift source was measured, not guessed: a **single** shift event whose sources include `nav.flex items-center gap-3 sm:gap-4 text-sm` — the storefront nav — and the same nav appears as a shift source in the **before** arm too (0.0017 / 0.0001). It is a pre-existing hydration shift in `components/storefront/storefront-nav.tsx`, magnified because there is now real content beneath the header to be pushed rather than a short skeleton.
- **The sitemap reversal, and why the recorded reason no longer holds.** The old comment excluded shop pages because "shop slugs are tenant data that would require a DB round-trip **at build time**." There is no build time here: the root layout is `force-dynamic` for the CSP nonce, this route now declares it too, and the slugs come from `/public/shops` — a public HTTP endpoint `/shop` already calls on every request. The base URL changed for a stronger reason than coverage: it was `NEXT_PUBLIC_SITE_URL ?? "http://localhost:3100"`, and `NEXT_PUBLIC_*` is inlined by Next at **build** time while this route built to `○ (Static)` — so the loopback fallback was frozen into the image. `k8s/scripts/check-env-contract.sh:210` carries that exact caveat as a known reviewed omission; routing through `resolvePublicOrigin()` (a plain runtime env, already required and already correct per overlay) closes it. Proven by construction: built with `NEXTAUTH_URL=http://localhost:3000`, started with `:3104`, and the sitemap emitted **:3104** — a build-time bake could not do that. **No hostname is written anywhere**; with no trustworthy origin the sitemap returns empty rather than advertising a guess at the unsettled production domain.
- **A second `isOpenNow` was quietly wrong, and server-rendering would have published it.** `/shop` still carried the pre-WR-05 implementation: it round-tripped through `toLocaleString("en-GB")` and re-parsed the result with the mm/dd-first `Date` parser, so the weekday row silently swapped for days 1–12 and was `Invalid Date` for 13–31; it also could not express an overnight window. Verified by running the old function against the new tests: it fails **2/2** of the calendar assertions (reports a shop OPEN on a Saturday it is closed, and CLOSED at 00:30 during Friday-night service). Left alone it would have moved from a client-only cosmetic bug into the crawled HTML and into `openingHoursSpecification`.
- **A soft 404 is only half-fixed, and the half that is not is recorded.** A missing slug renders `not-found.tsx` with `robots: noindex` — so a dead storefront cannot enter the index, which is the harm — but the response is still **HTTP 200**. `notFound()` was tried from the page body, from `generateMetadata`, and from both; app-wide `force-dynamic` streams the response and commits the status line before the tree finishes. Removing `app/shop/loading.tsx` (the obvious suspect) was tested and changed nothing. The root layout is shared with the dashboard and out of this change's scope.
- **"Works with JavaScript disabled" was asserted, failed against a correct tree, and became a measurement instead.** Next streams the body into `<div hidden>` blocks (4 of them, plus 3 `<template>`) that an inline script relocates into `<main>`. A consumer reading the response body gets the whole menu; a consumer rendering without scripting gets chrome only. Before this change neither got anything. The spec now records that distinction rather than rounding it up.
- **Two instrument defects were caught by their own fail direction and are documented in the spec.** A heading-order check scanning the raw HTML in *byte* order found two `<h3>` before the `<h1>` on a correct tree — they are the footer's, flushed first by streaming — so it reads the DOM instead. And a Playwright context created from `browser` does not inherit `baseURL`, which failed for a reason unrelated to the product.
- **A1's `main[aria-hidden]` assertion was over-specified, and the cause is quoted from the library.** `aria-hidden`'s `hideOthers` adds `[aria-live]` **and `script`** elements to its keep-set — *"as they have no impact on accessibility"* — which marks their ancestors as keep and makes the walk recurse into `<main>` rather than hide it. Adding the schema.org `<script>` therefore moved the attribute one node deeper onto main's content `<div>`, with nothing announced differently. Isolated with a single-variable arm on the same build: JSON-LD removed → **16/16** of `storefront-dish-modal-a11y.spec.ts` green; JSON-LD present → only those two blocks red. The assertion now measures the property ("the page behind the dialog is inert") instead of one node's attribute, and still fails pre-fix, where nothing was hidden at all.
- **A1's z-order invariant survived the split and is now guarded from both sides.** `<article>` keeps `relative`, the "Add" button and quantity stepper keep `relative z-10`, and the stretched trigger is still the article's **last** child — asserted in Jest (`server-seeded-islands.test.tsx`) as well as in A1's browser spec, so a future refactor cannot silently make the dish modal keyboard-unreachable again.
- **Heading order fixed as part of #447:** the dish title was an `<h4>` under an `<h2>` category, skipping a level. It is now `<h3>`; nothing visual changed, since size comes from the class.
...
#### Added
- **A printable kitchen ticket (#105).** There was no print flow anywhere in the product: zero `window.print` and zero `@media print` outside `components/marketing/business-model-guide.tsx`. A per-ticket button on every card and a `Print all` on the board render a 72mm monochrome ticket — the printable width of an 80mm thermal roll, and a sane narrow column on A4 — carrying shop, order reference at 22pt, fulfilment type, quantity-first line items, notes and the delivery address. No prices: a prep ticket is not a receipt.
- **A visible feed state and a last-updated stamp (#106).** `KdsFeedPill` (Live / Reconnecting / Offline / Not updating + a wall clock) and `KdsFeedBanner` (what is wrong, how old the tickets are, and a `Refresh now`).
- **`lib/kitchen-orders-api.ts`** — paged, bounded active-order fetch with a `truncated` signal the board renders.
- **`NEXT_PUBLIC_KITCHEN_ORDERS_PAGE_SIZE`** + `resolveKitchenOrdersPageSize` in `lib/env-validation.ts`, mirroring the `#282` shops knob.

#### Fixed
- **The board no longer claims to show All shops while showing one (#450 5d).** In the All-shops context it fell back to `shops[0]` and rendered that shop's tickets with nothing saying so. Measured on the live stack with three published shops: the sidebar read *All shops*, the board issued exactly one `?shopId=97d95aa4…` request, and every ticket on screen was Brixton Village Grill's. **Decision: label, do not aggregate.** A KDS is a screen in one kitchen, its live subscription is per shop (`/topic/kitchen.{tenant}.{shop}`), and merging would put one kitchen's orders in front of another's cooks. The board now names its shop always, and in the All-shops context says it is single-shop and how many shops are not on screen.
- **Two `size=100` fetches that discarded the tail (#485 `:175`, `:229`).** The shops fetch now reuses `fetchAllMyShops()` from #282. The orders fetch pages the list.
- **The e2e no longer self-skips when `/auth/signin` has no sign-in button.** That is the failure, not a reason to go green. The only remaining skip in the spec is `skipWithoutVendorPassword()`, which fires on a missing credential.

#### Notes
- **Raising `size=100` cannot fix #485 here, and that is measured, not assumed.** Against the running core-java for a shop with 125 orders: `size=100` → 100 rows, `totalPages 2`, `last false`; `size=500` → **the same 100 rows**. The server clamps the page size at 100, so the only fix available to a client is to follow `last`/`totalPages`. A shop whose history fits on one page still issues exactly one request; the 20-page circuit breaker sets a flag the board renders rather than truncating quietly.
- **The old connection indicator was not merely small — it was wrong.** With Playwright holding the browser offline for twelve seconds, the board displayed a green dot reading *"Connected"*: the STOMP client had not noticed its socket was dead and the page had no other source of truth. Liveness is now decided by `navigator.onLine` and by whether reads are landing (a 60s poll that either advances the stamp or does not), with the socket's opinion used only to name which failure it is.
- **`context.route()` cannot induce this.** It does not intercept WebSocket handshakes, so aborting a ws glob leaves the client happily connected — the before-state measurement did exactly that and read "Connected". `context.setOffline(true)` is what drops the socket.
- **A CLS regression was introduced and removed inside this change.** At the repo's declared throttle profile (375px `isMobile`, Fast-3G, 4× CPU, `webhooks-webperf.spec.ts`), `/dashboard/kitchen` measured **CLS 0.2408 before**. The first cut of the banner raised it to **0.7321** — it mounted on every cold load, because "the socket has not connected yet" is indistinguishable from "the socket is down" if you only look at `connected`, and then unmounted a second later, pushing the ticket grid down and letting it snap back. The banner is now suppressed until the board has either read once or failed once; the loading state keeps the header and reserves the grid's space; and the empty state no longer claims *"No active orders"* before orders have been read. **After: 0.2005** — better than before, and **still over the 0.1 budget.** The remaining 0.1698 is the loading placeholder being replaced by a full grid of tickets, which needs a skeleton sized to the eventual ticket count; that is out of scope here and unfiled.
- **Every acceptance criterion was run in the fail direction**, break → observe → restore-by-content → clean again: paging stopped after page 0 (3 fail), the `size=100` shops fetch restored (1 fail), liveness derived from the socket alone (1 fail), the pill label re-hidden behind `hidden sm:inline` (1 fail), the shop name removed from the header (4 fail), the All-shops notice removed (1 fail), the ticket quantity dropped (1 fail), the print sheet never mounted (2 fail). Both CLEAN brackets: 93 passed. The first run of that harness was itself vacuous — several jest path filters passed as one quoted string matched nothing and printed no `Tests:` line — so the runner now treats a missing summary as VOID.
### Four places the dashboard misdescribed a system that was already right (#534 — closes #454, part of #450) — 2026-08-04

Closes #454. Lands **4 of the 7** sub-items in #450 — items **1**, **2**, **5b** and **5c**; item 5d (the KDS "All shops" filter) belongs to the kitchen surface, and items 3 and 4 are backend, so #450 stays open.

#### Fixed
- **The shop-context switcher no longer renders on the per-tenant onboarding sub-tree (#450 item 1).** It was mounted on every dashboard route; onboarding never reads the context (it carries its own `shopId` for the application being created) and the QA council measured a switch there firing **0** API calls against a control arm on `/products` that re-fetched shop-scoped. The rule (`shopSwitcherApplies`) lives with the switcher, is prefix-matched so a future child inherits it, and returns `true` for an unknown pathname — an unreadable route must never remove working chrome. The saved context is untouched and is honoured again on leaving; which shop is active stays server-decided.
- **The staff page no longer promises an invite that does not exist (#450 item 2).** "…invite them to log in once" described a control the product does not have: `user_directory` is populated on first sign-in and the picker can only offer people already in it, so the one thing a group admin could not learn from this screen was what to do next. No invite flow is built here — that remains separate and decision-gated.
- **`#shop-context-select` identifies one node again (#450 item 5b).** The switcher is one control mounted twice, and **neither mount is ever removed** — each is hidden by a responsive class — so a single literal id put 2 matching nodes in every dashboard document (measured: **2 on all 13** routes). Invalid HTML, and `label[for]` resolved to whichever the parser reached first, which on a phone is the `display:none` sidebar copy. `useId()` was deliberately **not** used: the id is a published selector that `e2e/dashboard-mobile.spec.ts` locates the control by in three places, so the default top-bar variant keeps the canonical id and the sidebar takes the suffixed one. All 26 of that spec's tests still pass.
- **Every dashboard route carries its own `<title>` (#450 item 5c).** All 13 returned the root layout's `J'Toye OaaS - Multi-Tenant Order Management`. A page cannot fix this itself — they are Client Components and `metadata` is server-only — so 15 additive title-only `layout.tsx` files were added, plus a default on `app/dashboard/layout.tsx`. Titles are **absolute** rather than composed from a `title.template`, because the two `/dashboard/payments/connect` pages already ship finished titles and a template would suffix them twice.
- **CLS on `/dashboard/staff` (#454).** Measured **0.1805** twice at the repository's own declared profile (390px, Fast-3G, 4× CPU; budget `CLS < 0.1`, `frontend/e2e/webhooks-webperf.spec.ts:37`) — the worst route in the app. The cause was the loading state: a centred 128px spinner handing over to a ~1200px page. It is replaced by a loading state shaped like the page, built from the same `Card`/`Table` primitives so the geometry matches by construction rather than by copied pixel values, and rendering the static chrome — headings, descriptions, field labels, table columns — **for real**, with bars only where the data is genuinely unknown. Measured after: **0.0000** twice, at the same profile, and the budget was not relaxed.

#### Notes
- **The council's 0.1805 reproduced three times, on two different servers.** Twice on the branch dev server and once on the container build at `:3000` via the new spec's fail-direction run — so it is not a dev-mode artefact.
- **The instrument nearly lied first, in the direction of a pass.** The shared `core-java` container runs `CORS_ALLOWED_ORIGINS=http://localhost:3000,http://localhost:3100`, so a browser on any other port is refused twice over — the preflight, and then Spring's `CorsFilter` (403 on a disallowed `Origin`). Under that refusal the switcher falls back to its zero-grant state, `<select>` never renders, and `#shop-context-select` counts **0** — which reads as "the duplicate id is fixed" on a completely unfixed tree. The new spec therefore asserts **both** mounts render a `<select>` before counting, so the count cannot be 0 for the wrong reason.
- **Route-interception was the wrong CORS shim and the numbers said so.** Fulfilling API calls from the Playwright process bypasses CDP network throttling, so every dashboard route reported an identical **0.0188** — the shell's shift, with the page's own loading state never visible. Measurements were redone through a real socket.
- **Re-measured, #454's other two routes are no longer in breach**, at the same profile the finding used: `/` **0.0000** (council: 0.1213) and `/competitive` **0.0000** (council: 0.1040), 2 runs each. The issue was re-scoped to `/dashboard/staff` rather than carrying two unfalsifiable "fixes". The instrument was shown able to see a real shift on the same run — `/dashboard/kitchen` reports **0.1471**.
- **`/dashboard/kitchen` is over the budget at 0.1471, before and after, and is not fixed here** — it belongs to another surface. Every other route measured is unchanged to 4 decimal places.
- **Falsifiability.** Four Jest break arms (single literal id; `shopSwitcherApplies` always true; the invite copy; the spinner) each fail the suite and each restore was verified by `git hash-object`, bracketed clean → arms → clean. The new Playwright spec was run against the pre-fix container build at `:3000`: **5 failed, 0 passed**.
- **The new spec is `@mobile-only`, deliberately.** Every block pins its own 390px `isMobile` viewport, so the desktop project would measure the same thing twice — while doubling the load on an API that rate-limits at 100 requests/minute per tenant. Measured: back-to-back project runs exhausted the bucket and the duplicate-id block failed for a reason unrelated to the code under test.
### The storefront dish modal becomes a real dialog, and the basket stops saying "1 items" (#533 — closes #446, #272) — 2026-08-04
...
#### Added
- **`scripts/check-postgres-major-parity.sh`** — the PostgreSQL major of the backup **tooling** must equal the **server's**, across the 8 sites declared in `scripts/gates/postgres-major-parity.conf` (3 servers, 3 tooling pins, and both EOL rows in `infra/dependency-horizons.yaml`). Wired into the `ops-contracts` CI job. **It does not forbid PostgreSQL 18 — it forbids the halves diverging.**

#### Notes
- **What prompted it.** Dependabot #525 bumped `infra/backups/Dockerfile` from `postgres:15-bookworm` to `18-bookworm` while every server in the tree stayed on 15, and **every CI check was green**. Measured against the live 15.17 server: a `postgres:18` `pg_dump -Fc` succeeds (469,421 bytes), and `postgres:15` `pg_restore --list` then refuses it — `unsupported version (1.16) in file header`. The control (`postgres:15` reading a pg15 dump) is rc=0, and the direction check (`postgres:18` reading a pg15 dump) is rc=0. So tooling reads its own major and **older, never newer**: a backup image ahead of its server keeps taking backups that succeed and are unrestorable by the server's own client, surfacing only during a recovery.
- **A comment cannot fail a build.** `infra/backups/Dockerfile` already said, two lines above the line dependabot changed, *"The tag must match the image referenced in k8s/base/pg-backup-cronjob.yaml."* This script is that sentence made executable.
- **The backup script's own verification is structurally blind to this.** `k8s-backup.sh:66` runs `pg_restore --list` **inside the image that produced the dump**, so pg18 tooling reads a pg18 archive and always agrees. It agrees **by construction** — the same shape as the edge↔core `processed_count` bug #337 found, where both sides encoded with the same struct. A check outside that loop is the only place the question can be asked.
- **Anchored sites, not a bare token — and both reasons were found by running the extractor over the real files.** `docker-compose.full-stack.yml:130` contains `jdbc:postgresql://postgres:5432/keycloak`, from which a bare `postgres:` token extracts **5432** as a major. `infra/backups/Dockerfile:4` contains `postgres:15-alpine` inside a **historical** comment describing the approach that was replaced — it must keep saying 15 forever, so matching it would fail this gate on a *correct* tree the day the server moves to 18. "Skip comment lines" would have been the wrong rule too, because `Dockerfile:8`'s build tag **is** in a comment and **is** load-bearing.
- **Nine break arms, and the two that mattered most were not the obvious ones.** Arm 6 upgraded **every** site 15→18 and asserted **rc=0** — a gate that blocks a correct upgrade is worse than no gate, and this is also what proves the JDBC line and the historical comment are not false-positive sources (both still read 15 while the gate is green). Arm 9 moved all three tooling pins to 18 with servers at 15 to exercise the **cross-role** comparison, which arms 1–4 never reached because they tripped the intra-role check first.
- **Two break arms were vacuous on their first run and had to be redone.** Passing the config on `/dev/stdin` exited 2 with `config not found` — the `[ -f ]` test rejects a pipe — so both VOID arms were "passing" for a reason unrelated to what they tested. Re-run against real temp files, they VOID for the right reasons: a declared site whose anchor matches nothing, a declared file that does not exist, and a roster with no SERVER row.
- **`grep`/`rg` are shell functions on this machine and do not exist inside an exec'd script** — a bare `grep` there returns 127, which reads exactly like "found nothing". Extraction uses `awk`, a real binary.
- **What a green run does NOT mean.** It proves the *declared* majors match. It does not run `pg_dump`, does not test a restore, and cannot tell you the backups are recoverable. `docs/runbooks/backups.md` documents the procedure; **nothing in this repo executes a restore drill**, and the gate says so on every pass.

#### Added — the restore drill (the first one this repo has ever run)
- **`scripts/restore-drill.sh`** — dumps with the tooling `infra/backups/Dockerfile` declares, as the BYPASSRLS role the CronJob uses, restores into a **throwaway server of the deployed major**, and compares per-table row counts plus the Flyway migration count. First execution, 2026-08-04: **40 tables / 8095 rows / flyway 60/60, restored clean.** The 2026-07-08 remediation backlog (P1-8) recorded *"no restore ever rehearsed"*; that line was still true this morning. Deliberately **not** named `check-*` — it needs a live DB and Docker, so on a runner it could only VOID (same reason `check-runtime-freshness` stays out of CI), and the naming keeps it out of the gate count (precedent: `ci-lane-cost.sh`).

#### Notes — the restore drill
- **`pg_restore --list` was never evidence of a restorable backup.** It reads the archive *header* and loads zero rows, so a structurally-valid archive containing nothing passes it. That is what `k8s-backup.sh:66` does, and it runs inside the image that produced the dump, so it agrees by construction twice over.
- **The trap the script is built around is that RLS blinds the verifier.** Most tables are ENABLE + FORCE RLS; a count run as a non-BYPASSRLS role with no tenant GUC returns fewer rows silently, with exit status 0. Count both sides that way and the comparison is `0 == 0` — a PASS over a restore that loaded nothing, the same shape as the `isZero()` assertion this repo has already been bitten by. Three defences: both sides counted as BYPASSRLS; a source-row floor (`MIN_ROWS`) so zero-vs-zero can never pass; and a **control** that proves the blind method really is blind here before the sighted one is trusted. Measured on `media_asset_aud`: BYPASSRLS sees **2224**, unpinned app role sees **741**.
- **The drill found a real ambiguity on its first run, and it was in the drill, not the product.** `DB_USER` means two different things: `.env` `DB_USER` is `jtoye_app` (the application role, **not** BYPASSRLS), while the CronJob's `DB_USER` comes from the secret key **`backup-username`** = `jtoye_backup` (BYPASSRLS). The first draft used `.env`'s value and `pg_dump` failed with *"query would be affected by row-level security policy for table customers"*. That reads like a production fault and is not one — `k8s/base/pg-backup-cronjob.yaml:68` sources the other key. The role attribute is now **asserted before dumping**, so the failure names a role instead of a table.
- **Six break arms, clean → arms → clean.** `MIN_ROWS` floor → VOID; non-BYPASSRLS role → VOID; absent role → VOID; absent source container → VOID; **pg18 tooling against a pg15 target → rc=1 with `restored: 0 rows`**; clean re-run → rc=0. No throwaway containers leaked (trap cleanup), and the source database was untouched.
- **The pg18 arm is the one that ties the two halves together.** It reproduces dependabot #525 end-to-end and is caught **by content** — `pg_restore`'s own exit code is deliberately treated as non-fatal (it exits non-zero on benign warnings), so the row comparison is the arbiter. That also means a *partial* restore is caught, which an exit-code check would miss.
- **A witness was wrong and had to be corrected.** The Flyway check first used `max(version)`, which is a **TEXT** column — so it sorts `'9'` above `'60'` and reported "flyway 9" on a V60 schema. It matched on both sides so it produced no false failure, but it was reporting a meaningless number. Now `count(*)` + `max(installed_rank)`.
- **The runbook's own recipe was superseded, with the reasons recorded rather than deleted.** It restored into the **live** container, used `gunzip | psql` against a pipeline that writes `-Fc` custom format (so it could not have opened the real artifact), proved success with `\dt | head` — a truncating filter that lists table names and never counts rows — and said nothing about RLS.
- **Still not proven:** row COUNTS, not row contents. No column-value comparison, no application boot against the restored database, no PITR, no cross-major restore, and the source is the dev database rather than the real S3 artifact. The script prints these caveats on every pass.
