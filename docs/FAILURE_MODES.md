...
| **F-0.2** | **`/api/v1/sync/batch` is a within-tenant authorization gap (BOLA).** Gated only by `authenticated()` — no scope, no role, no `ShopAccessService` check. Any tenant staff member scoped to shop A can upsert **any** shop/product in their tenant, bypassing the Phase-23 second wall. Its only test runs `@WebMvcTest(addFilters=false)` — zero authz coverage. Cross-tenant reach is blocked by RLS; the gap is intra-tenant, write-side. | CODE `SecurityConfig` (`anyRequest().authenticated()`); `SyncService` writes via repositories with no access check; `SyncControllerIntegrationTest` disables filters. Live caller: edge-go `main.go:304` (no production producer exists). | **CLOSED 2026-09-02 — QA-council 20260902 Cluster A (#648; findings API-1 / API-2 / SEC-5 / API-13).** `SyncController.batchSync` now rides `@PreAuthorize("hasAuthority('SCOPE_catalog:write')")` (the machine half, parity with `ProductController`); `SyncService` gates every resolved product/shop on its OWNING shop via `ShopAccessService.require(shopId, SHOP_MANAGER)` and routes a shop create through `ShopService.createShop` (GROUP_ADMIN-only, slug derived, never born published); each item carries the `CreateProductRequest` bounds. Deny tests were written FIRST and watched fail: `SyncBatchAuthorizationIntegrationTest` (real filter chain, Testcontainers). |
...
| F-3.2 | **Orders complete with no payment.** The COD fallback violates the owner's 2026-08-02 ruling (payment link to a verified phone). | `PublicStorefrontService.java:907` `paymentMethod="Cash on Delivery"`. | Open, **#461** (P1); Phase 30 not started. |
