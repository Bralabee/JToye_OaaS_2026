...
| E2E | **The authority is the NIGHTLY, and it is GREEN: `180 total / 173 passed / 0 failed / 7 skipped`** on `d4930719`, twice (§0.-6). ⚠ **The local figure below is from 2026-08-04 and is NOT re-measured at `7c1ef2a7`** — `check-e2e-skip-budget` is rc=2 VOID precisely because the stored report predates the spec changes that arrived with main, so the local suite needs a re-run (`set -a; . ./.env; set +a`, then `scripts/seed-e2e-fixtures.sh`, then the playwright command in §6 — ~10+ min). Do not quote the local number as current. STALE LOCAL ROW FOLLOWS — **127 passed / 8 skipped / 0 failed of 135** — a LOCAL run of the spec files, NOT the nightly (which runs both projects: 180 instances, see §0.-5) —, run against the re-synced stack, `check-e2e-skip-budget` **rc=0** at exactly its ceiling of 8. ⚠ **The first run of this suite reported 48 skipped / 21 undeclared and that figure was an INSTRUMENT ARTEFACT, not a finding** — the suite was launched without sourcing `.env`, so 26 vendor-authenticated specs self-skipped on "No vendor password". `set -a; . ./.env; set +a` first, and export `E2E_VENDOR_PASSWORD` from `KC_SEED_USER_PASSWORD`. A skip count is meaningless unless the credentials were present |
