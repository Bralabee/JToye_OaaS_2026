...
- **`check-test-count-oracle.sh` requires a mode argument.** A bare call exits **2 (VOID) on a usage
  error**, indistinguishable from a real VOID. §6 still prescribes a bare sweep; that instruction is
  incomplete. With `jest` / `playwright` / `vitest` it is rc=0 three times.
