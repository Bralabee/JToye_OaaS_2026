...
`STOMP_BROKER_MODE: ${STOMP_BROKER_MODE:-in-memory}` and `core-java/src/main/resources/application.yml:386` reads
...
`mode: ${STOMP_BROKER_MODE:in-memory}` (`core-java/src/main/resources/application.yml:385-388`), so this code path is exercised **only** on this cluster (D-06).
