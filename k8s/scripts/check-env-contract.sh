...
ALLOW_FE_B=(
  ...
  'CORE_API_INTERNAL_URL|Reviewed omission: optional server-side override for the pod-to-Core base URL (frontend/lib/customer-orders-server.ts:43-49). Unset, it falls back to the build-time NEXT_PUBLIC_API_URL, which in k8s is a routable ingress origin rather than compose s unreachable localhost:9090 — so absence costs a hairpin through the ingress, not a 502. Worth supplying when the frontend gets a per-environment runtime config; not a silent failure today.'
