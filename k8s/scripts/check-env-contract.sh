...
ALLOW_FE_B=(
  ...
  'CORE_API_INTERNAL_URL|Reviewed omission: optional server-side override for the pod-to-Core base URL (frontend/lib/customer-orders-server.ts:44-49). Unset, it falls back to the build-time NEXT_PUBLIC_API_URL, which in k8s is a routable ingress origin rather than compose s unreachable localhost:9090 — so absence costs a hairpin through the ingress, not a 502. Worth supplying when the frontend gets a per-environment runtime config; not a silent failure today.'
  ...
  'NEXT_PUBLIC_KEYCLOAK_URL|Reviewed omission with a known caveat, and the one NEXT_PUBLIC_* name that is deliberately RUNTIME-resolvable: frontend/Dockerfile documents (with measured evidence) that giving it an ARG/ENV line would freeze it to "" in the bundle, so it has none, and docker-compose supplies it as a runtime environment: value instead. No k8s manifest does. Consequences are bounded but real: it is in env-validation.ts requiredEnvVars, so every k8s pod logs a missing-required-var error at boot, and frontend/middleware.ts:24 builds the enforcing CSP with keycloakOrigin "". The vendor login still works because auth.ts:48 falls kcPublicBase back to KEYCLOAK_ISSUER, which IS injected. Supplying it needs a frontend runtime-config decision, not a one-line manifest edit.'
