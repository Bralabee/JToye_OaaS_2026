...
ALLOW_EDGE_B=(
  ...
  'EDGE_MANAGEMENT_PORT|Reviewed omission, and still one that MUST STAY AN OMISSION — but NOT for the reason recorded before issue #550. The old reason was "the Phase 27 scrape config targets the app port with no credentials", and that is now FALSE for compose: #550 sets EDGE_MANAGEMENT_PORT on the edge-go service in docker-compose.full-stack.yml and moves the scrape target with it, both from the single .env key EDGE_GO_METRICS_PORT. The k8s conclusion is unchanged and the k8s reason is now STRONGER than the one recorded here before, which plan 29-06 falsified (DEF-29-2). That reason said "NOTHING SCRAPES EDGE-GO IN K8S AT ALL. k8s/ ships zero monitoring manifests (DPLY-03) — no Prometheus, no ServiceMonitor", and all three clauses are FALSE on this tree: k8s/base/kustomization.yaml renders monitoring/prometheus-deployment.yaml, and that Prometheus scrapes edge-go on /metrics at the static target edge-go:8080 (k8s/base/monitoring/prometheus-config.yaml:221-231) — precisely the port the prometheus.io/scrape+port+path annotations on k8s/base/edge-go-deployment.yaml:26-28 advertise. So the variable must STILL stay unset, for a better reason than "no scraper": the edge serves /metrics on EXACTLY ONE of its two ports (registerAppMetricsRoute registers the app-port route only while EDGE_MANAGEMENT_PORT is empty), so supplying a port here would move /metrics off 8080 and turn a scrape that NOW EXISTS into a permanently-DOWN target, as well as making those three annotations lie. Whoever changes it must change this variable AND the annotations AND the scrape target as ONE change, exactly as compose does from EDGE_GO_METRICS_PORT. Do not "fix" this entry by supplying the variable on its own.'
