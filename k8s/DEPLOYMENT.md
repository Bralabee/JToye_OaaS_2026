...
| **`scripts/check-postgres-major-parity.sh`** | **`ops-contracts`** | The PostgreSQL major of the backup **tooling** equals the major of the **server**, at every site that declares either (8 today, listed in `scripts/gates/postgres-major-parity.conf`). Added after dependabot #525 bumped `infra/backups/Dockerfile` to `postgres:18` while every server stayed on 15 — with **every CI check green**. `pg_dump`/`pg_restore` read their own major and older, never newer: measured against the live 15.17 server, a `postgres:18` dump is rejected by `postgres:15` `pg_restore` with `unsupported version (1.16) in file header`, so backups keep succeeding and silently stop being restorable by the server’s own client. `k8s-backup.sh`’s own `pg_restore --list` cannot catch it — it runs *inside* the image that produced the dump and agrees by construction. This gate does **not** forbid a newer PostgreSQL; it forbids the halves diverging. A declared site that matches nothing is a **VOID (2)**, never a silent pass. |
