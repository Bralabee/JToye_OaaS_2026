...
| **O-2** | Record a dated horizon row for the `snackpass-pg` free-window expiry: **~2027-07-21** (creation 2026-07-21 + 12 months), impact ~£21/month B1ms, **exact date to be confirmed from Azure Portal → Cost Management → Credits + offers**. | horizon row in `infra/dependency-horizons.yaml` | It breaches the £150 ceiling with **no deploy and no code change**. Unrecorded, it is discovered on an invoice. |
| **O-3** | Carry the PG16-vs-PG15 skew in ADR-0002 with its evidence, naming both the `CLAUDE.md` PostgreSQL 15 line and the `postgres:15-alpine` compose pin it diverges from. | 29-01 Task 3 | A version skew that lives only in a SKU argument is indistinguishable from an accident. |
