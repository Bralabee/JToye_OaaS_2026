...
Last activity: 2026-07-09 - Completed quick task 260709-bl2: Issue #87 P1-5 JWT audience + realm hardening — core-java additive AudienceValidator (DelegatingOAuth2TokenValidator: issuer+audience+timestamp, fail-closed on blank config, expected-audience env-injected) also strengthening previously-timestamp-only issuer check; edge-go fail-closed audience (default core-api, no inert path); realm bruteForceProtected+passwordPolicy(len12)+core-api oidc-audience-mapper on template; refreshToken removed from client NextAuth session (buildSession) kept on server JWT; 3-suite gate green (452 core-java unit + 100 integration + edge-go 6 pkgs + 104 Jest, docs 755). #83-#87 done (#88 remains). RUNTIME FOLLOW-UP: realm audience mapper needs Keycloak DB drop + re-import to emit aud=core-api; until then live tokens are (correctly) rejected fail-closed.
