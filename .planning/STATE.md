...
Last activity: 2026-07-08 - Completed quick task 260708-tsl: Issue #86 P1-4 Redis resilience — RedisCacheErrorHandler degrades cache GET/PUT (WARN) + EVICT/CLEAR (ERROR, staleness-alarmed) to source-of-truth, explicit Lettuce command timeout from per-profile spring.data.redis.timeout, rate-limiter fail-open-with-alarm (jtoye.ratelimit.fail_open counter) + jtoye.cache.errors metric; +RedisFaultInjectionIntegrationTest (stops Redis mid-test) + unit tests; full gate green (447 unit + 100 integration, docs 746). #83-#86 done (#87-#88 remain).
