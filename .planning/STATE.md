...
Last activity: 2026-07-08 - Completed quick task 260708-teb: Issue #85 P1-3 guest-checkout stock — double-decrement empirically CONFIRMED then converged to a single retry-safe decrement at CONFIRM (eager guest-path decrement removed); +GuestCheckoutStockConvergenceIntegrationTest (A: delta 1×qty, B: concurrent no-500) + StockDecrementLocationTest guard; full gate green (739 invocations). #83-#85 done; #86 planned/parked (#87-#88 remain). NOTE: #85 executor hit account session limit mid-gate — recovered inline.
