...
- [x] **CUST-01**: Locality exists as a concept and nothing on the storefront is fictional — device location used, shop coordinates read, a delivery radius enforced, and "Cooking near you" resolving to real published shops. Closes #460 and #544. **Satisfied 2026-08-09**, each limb against evidence that was actually captured rather than against a plan that said it would be:

  | Limb | Closed by | Evidence, and the break that proves it could fail |
  |---|---|---|
  | Nothing on the row is fictional | `33-03` | The five invented vendors, invented "⭐ 4.8" ratings and invented "FHRS 5" badges are gone; the row renders the REAL published shops the seeder creates. Control: the invented names measured 0 files across `core-java/src/main`, the real "Mama Ade" measured 3 |
  | Shop coordinates are read | `33-05` | `latitude`/`longitude` populated from Code-Point Open and closed **against the delivered runtime under both database roles**, not against a test fixture (`33-05` Task 4) |
  | A delivery radius is enforced | `33-06` + `33-07` | `GET /public/shops?lat=&lon=&radiusKm=`, exact-radius predicate in a derived table over a leakproof box. The CORNER fixture sits inside the box at ~1.27r and is absent at 5 km / present at 8 km — the only fixture that distinguishes "filtered by a box" from "filtered by a radius". The customer observes it as the honest third state |
  | Device location is used | `33-07` | Gesture-gated `getCurrentPosition`, one request per grant, three states derived from the coordinate. Deterministic in Playwright via `grantPermissions` + `setGeolocation`, all four arms (denied / granted / far-away / exclusion) green against the rebuilt runtime |
  | The distance ordering is real | `33-06` + `33-07` | The list **REORDERS when the caller moves**: from Brixton `brixton-village-grill` is first, from Peckham `mama-ades-kitchen` is. A merely-sorted list is indistinguishable from name or insertion order; only the inversion can be driven by the caller's position. Pinned by slug to the ~2 km pair, never the ~600 m Peckham pair |
  | An excluded shop is disclosed, not dropped | `33-07` | A published shop with no coordinate is named in a count with a route to the full list; the E2E arm NULLs a real shop's coordinates on the live database, restores them, and verifies the restore by content. Observed failing with the disclosure removed |

  Distances and the radius are shown to the customer in **miles** (the wire stays metric); corrected at the human-verification gate on 2026-08-09 after the owner walked the full journey.
- [ ] **CUST-02**: No lifecycle dead-ends for a consumer or a vendor — onboarding `MANUAL_REVIEW` reaches a surface a human can act from (or a recorded decision names who adjudicates it, given there is no cross-tenant operator identity), signed-in customer nav is gated, tracking moves into the profile, and second-shop onboarding plus staff invite exist. Closes #453, #458, #452. **NOT closed, and deliberately left unticked.** Its #453 limb is carved out by Phase 33's decision D-2 as a decision ticket; #458's dispatch half and #452 are out of scope per D-3. Stated plainly rather than left to be inferred: D-2's recorded rationale explains why nothing was built, but **no adjudicator is named** — and naming who adjudicates `MANUAL_REVIEW` is the second limb this requirement asks for, so the recorded decision does not satisfy it either. There is no cross-tenant operator identity to fall back on. This gap is the one the ROADMAP escalated and it survives the phase open.
...
- [ ] **CUST-04**: The customer-facing surface has been reviewed against what it actually renders — a look-and-feel pass on web and mobile, Keycloak stops shipping the stock theme on both realms, and the staff screen gains bulk-revoke of JIT-provisioned rows. Closes #546, #545, #285. A screenshot cannot verify motion. **Out of scope per D-3 and NEVER MEASURED — recorded as unknown, not as clean.** No plan in Phase 33 looked at the Keycloak theme on either realm or at the staff screen's bulk-revoke, so nothing here is evidence of anything. An unmeasured requirement reported as "not started" and an unmeasured requirement reported as "fine" look identical from the outside; this one is the former.

#### Two product limitations of the located row, recorded now so they are never later discovered as bugs

Both are properties of the coordinate source (`33-02` D-1, Ordnance Survey Code-Point Open), not defects, and neither has a fix inside this phase:

1. **Coordinates are postcode-centroid accurate — about 100 m — and NOT door-level.** A shop's pin is the centre of its postcode unit, so two shops sharing a postcode are zero metres apart as far as the ranking is concerned. This is why distance is printed to a tenth of a mile (161 m, coarser than the error) and never in yards: a finer figure would advertise a precision the data does not have. It is also why the E2E ordering assertion is pinned to the ~2 km Brixton/Peckham pair rather than the ~600 m Peckham pair, which is inside centroid noise.
2. **Coverage is Great Britain only.** Code-Point Open is a GB product: it does not include Northern Ireland. A Northern Ireland vendor will never geocode, **keeps their storefront and their published status**, and is absent from every distance-ranked result — but is DISCLOSED by `33-07`'s exclusion notice with a route to the full list, rather than silently vanishing from the platform's primary discovery row. The same applies to any postcode newer than the committed snapshot.

#### Found at the human-verification gate, filed rather than built

The owner walked the full located journey on 2026-08-09 and approved it. Two search findings surfaced during that walkthrough, both verified against the live stack, both **outside CUST-01's scope**, and both filed with the owner's explicit agreement rather than absorbed into this phase:

- **A customer postcode with no string match returns zero shops** — e.g. `SE22`, which no seeded shop's address contains. Filed as **#619** (postcode-proximity search). The full-text path itself is healthy and was measured so: `SE15` → 2 shops, `SE15 5BS` → 1, `jollof` → 2, all correct. The gap is that search is not coordinate-aware — `33-06` deliberately refuses `q` combined with `lat`/`lon` rather than silently ignoring one of them, so "search near me" is a new capability needing its own decision, not a parameter.
- **Food-term search is string matching only, with no semantic understanding** — a customer searching for a dish by a synonym finds nothing. This is the open pgvector track, **#207**, unchanged by this phase.
...
| CUST-01 | Phase 33 | 33-01, 33-02, 33-03, 33-05, 33-06, 33-07 | **Complete** — 2026-08-09, per-limb evidence above. Coordinates (33-05, closed against the delivered runtime), the distance query (33-06), the real-shop row (33-03) and the located journey (33-07, human gate approved). Limitations recorded: ~100 m centroids, Great Britain only |
| CUST-02 | Phase 33 | 33-04 (D-2 decision only) | **Not closed** — #453 carved out as a decision ticket, #458 dispatch half and #452 out of scope per D-3. The recorded decision explains the non-build but names no adjudicator for `MANUAL_REVIEW`, which is the requirement's own second limb. Gap survives the phase open |
...
| CUST-04 | Phase 33 | not planned — out of scope per D-3 | **Unknown, not clean** — never measured by any plan in this phase |
