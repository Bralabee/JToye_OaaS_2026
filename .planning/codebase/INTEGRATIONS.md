...
- Stripe — Online card payments + COD fallback
  - SDK: `com.stripe:stripe-java:33.3.0` (`core-java/build.gradle.kts:150`)
...
- Ollama (default) — Local GPU-accelerated vision model
  ...
  - GPU: NVIDIA device reservation in `driver: nvidia` in `docker-compose.full-stack.yml:676-680`
...
**Databases:**
- PostgreSQL 15-alpine (`docker-compose.full-stack.yml:43`)
...
- MinIO (S3-compatible, local dev) / AWS S3 (prod)
  ...
  - Bucket: `S3_BUCKET` (`jtoye-images`), anonymous GetObject-only policy (#626) applied by `minio-init` sidecar (`docker-compose.full-stack.yml:567-583`)
  - Public URL: `S3_PUBLIC_URL`
  - SDK: AWS SDK v2 BOM → `software.amazon.awssdk:s3` (`core-java/build.gradle.kts:124-125`). Version is claimed once, in STACK.md, which the version gate checks.
...
- Image: `rabbitmq:4.3.4-management-alpine` (`docker-compose.full-stack.yml:224`)
...
- Java client: `spring-boot-starter-amqp` (`core-java/build.gradle.kts:57`)
- Use cases:
...
- `spring-boot-starter-oauth2-resource-server` (`core-java/build.gradle.kts:96`)
...
**Distributed Tracing:**
- Micrometer Tracing with Brave bridge → Zipkin reporter (`micrometer-tracing-bridge-brave` in `core-java/build.gradle.kts:143`)
