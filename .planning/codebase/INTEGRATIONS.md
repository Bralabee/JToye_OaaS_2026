...
- Ollama (default) — Local GPU-accelerated vision model
  ...
  - GPU: NVIDIA device reservation in `driver: nvidia` in `docker-compose.full-stack.yml:567-569`
...
- MinIO (S3-compatible, local dev) / AWS S3 (prod)
  ...
  - Bucket: `S3_BUCKET` (`jtoye-images`), public-read policy applied by `minio-init` sidecar (`docker-compose.full-stack.yml:489-503`)
