...
- Ollama (default) — Local GPU-accelerated vision model
  ...
  - GPU: NVIDIA device reservation in `driver: nvidia` in `docker-compose.full-stack.yml:669-673`
...
- MinIO (S3-compatible, local dev) / AWS S3 (prod)
  ...
  - Bucket: `S3_BUCKET` (`jtoye-images`), anonymous GetObject-only policy (#626) applied by `minio-init` sidecar (`docker-compose.full-stack.yml:560-576`)
...
- Image: `rabbitmq:4.3.4-management-alpine` (`docker-compose.full-stack.yml:217`)
