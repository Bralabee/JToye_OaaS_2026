...
- Ollama (default) — Local GPU-accelerated vision model
  ...
  - GPU: NVIDIA device reservation in `driver: nvidia` in `docker-compose.full-stack.yml:528-530`
...
- MinIO (S3-compatible, local dev) / AWS S3 (prod)
  ...
  - Bucket: `S3_BUCKET` (`jtoye-images`), public-read policy applied by `minio-init` sidecar (`docker-compose.full-stack.yml:443-456`)
...
- `mailhog/mailhog:v1.0.1` (`docker-compose.full-stack.yml:552`)
